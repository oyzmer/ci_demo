<div class="btn-group">
    <a href="<?php echo site_urlc('video/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
require(['adminer/js/ui'],function(ui){
    var video = {
        url_del: "<?php echo site_urlc('video/delete'); ?>"
        ,url_audit: "<?php echo site_urlc('video/audit'); ?>"
        ,url_flag: "<?php echo site_urlc('video/flag'); ?>"
        ,url_sortid: "<?php echo site_urlc('video/sortid'); ?>"
        ,url_sort_change: "<?php echo site_urlc('video/sort_change'); ?>"
        ,url_copy: "<?php echo site_urlc('video/copypro'); ?>" 
    };
    ui.fancybox_img();
    ui.btn_delete(video.url_del);     // 删除
    ui.btn_audit(video.url_audit);    // 审核
    ui.btn_flag(video.url_flag);      // 推荐
    // ui.sortable(video.url_sortid); // 排序  拖动排序和序号排序在firefox中有bug
    ui.sort_change(video.url_sort_change); // input 排序
    ui.btn_copy(video.url_copy);    // 热门审核
});
</script>

...
require(['adminer/js/ui'],function(ui){
	var links = {
		...
		,url_flag: "<?php echo site_urlc('links/flag'); ?>"
		,url_sortid: "<?php echo site_urlc('links/sortid'); ?>"
		,url_sort_change: "<?php echo site_urlc('links/sort_change'); ?>"
		,url_copy: "<?php echo site_urlc('links/copypro'); ?>" 
	};
	ui.fancybox_img();
	ui.btn_delete(links.url_del);		// 删除
	ui.btn_audit(links.url_audit);	// 审核
	ui.btn_flag(links.url_flag);		// 推荐
	// ui.sortable(links.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(links.url_sort_change); // input 排序
	ui.btn_copy(links.url_copy);    // 热门审核
});
</script>

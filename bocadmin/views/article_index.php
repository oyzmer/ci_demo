<div class="btn-group">
	<?php if($this->ccid!=0){ ?>
	<a href="<?php echo site_url('article/index/'.$this->cid)?>" class='btn btn-info' > <i class="fa fa-arrow-left"></i> 返回栏目 </a>
	<?php } ?>
...
<table class="table table-striped table-hover select-list">
	<thead>
		<tr>
			...
			<?php if($this->cid!=14) {?><th>图</th><?php } ?>
			<th>排序</th>
			<th>标题</th>
	...
	<tbody class="sort-list">
		...
		<tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
			...
			<td>
				<div class="btn-group">
					<?php if($this->cid!=14) {?><?php include 'inc_ui_flag.php'; ?>

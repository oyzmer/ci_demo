<div class="btn-group">
    <a href="<?php echo site_urlc('files/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
require(['adminer/js/ui'],function(ui){
    var files = {
        url_del: "<?php echo site_urlc('files/delete'); ?>"
        ,url_audit: "<?php echo site_urlc('files/audit'); ?>"
        ,url_flag: "<?php echo site_urlc('files/flag'); ?>"
        ,url_sortid: "<?php echo site_urlc('files/sortid'); ?>"
        ,url_sort_change: "<?php echo site_urlc('files/sort_change'); ?>"
        ,url_copy: "<?php echo site_urlc('files/copypro'); ?>" 
    };
    ui.fancybox_img();
    ui.btn_delete(files.url_del);     // 删除
    ui.btn_audit(files.url_audit);    // 审核
    ui.btn_flag(files.url_flag);      // 推荐
    // ui.sortable(files.url_sortid); // 排序  拖动排序和序号排序在firefox中有bug
    ui.sort_change(files.url_sort_change); // input 排序
    ui.btn_copy(files.url_copy);    // 热门审核
});
</script>
